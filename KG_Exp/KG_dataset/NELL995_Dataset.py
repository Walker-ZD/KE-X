...
class NELL995_Dataset(InMemoryDataset):

    def __init__(self, root: str, relation_id: int,
                 transform: Optional[Callable] = None,
                 pre_transform: Optional[Callable] = None):

        assert relation_id in range(200)
    ...
    def raw_dataset_dir(self) -> str:
        return osp.join(self.raw_dir, '../../nell995')
    ...
    def download(self):
        # print("Enter NELL995 Download function")
        return
    ...
    def process(self):
        ...
        train_edge_index = torch.tensor(edge_index, dtype=torch.long)
        
        for node in node_list_train:
            if node_dict[node] <= train_edge_index.max().item():
    ...
    def __repr__(self) -> str:
        return f'{"NELL995_" + self.relation_name}()'
