...
def Saliency_Map_mask(target_node, model, data, id_entity=None, edge_relation=None):
    ...
    for index in range(grad.shape[0]):
        if index == target_node: 
            ...
            grad_list.append(torch.norm(F.relu(grad[index]), p=1).item())
...
def calculate_SM_Metrics(target_node, model, data, node_mask_score, threshold=0.1):
