...
from KG_Exp.KG_dataset import NELL995_Dataset
...
def test():
    ...
    if 'max_acc' in globals():
        path = osp.dirname(osp.realpath(__file__)) + "/../../model_save/nell995_relation" + "_" + str(args.relation_id) + "_" + args.model + "_conv.pt"
...
path = osp.join(osp.dirname(osp.realpath(__file__)), '..', 'data', 'nell995_' + str(args.relation_id))
dataset = NELL995_Dataset(path, args.relation_id)
data = dataset[0]
...
if args.action == 'explain':
    ...
    if os.path.exists(path) == False:
        print("No model for " + str(args.relation_id))
        sys.exit(0)
    ...
    save_path = osp.dirname(osp.realpath(__file__))  + "/../../explain_result/nell995_relation" + "_" + str(args.relation_id) + "_" + args.model + "_explain_result.json"
    ...
    result_path = osp.dirname(osp.realpath(__file__))  + "/../../explain_result/nell995_relation" + "_" + str(args.relation_id) + "_" + args.model + "_explain_result.json"
    with open(result_path, 'r') as f:
        node_dict = json.load(f)
        
    calculate_metric(node_dict)
