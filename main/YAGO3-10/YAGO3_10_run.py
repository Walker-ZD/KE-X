...
from KG_Exp.KG_dataset import YAGO3_10_Dataset
...
class Net(torch.nn.Module):
    def __init__(self, ent_embedding, rel_embedding, relation_num, class_num, conv):
        ...
        self.rel_weight = nn.Parameter(torch.rand([300, 50]))

    def forward(self, x, edge_index, edge_type, target_node=None):

        if target_node == None:
            print("target node set is none\n")
            return
...
path = osp.join(osp.dirname(osp.realpath(__file__)), '..', 'data', 'yago3-10_' + str(args.relation_id))
dataset = YAGO3_10_Dataset(path, args.relation_id)
data = dataset[0]
...
if args.model == 'distmult':
    ...
    ent_embedding = torch.tensor(np.load(osp.dirname(osp.realpath(__file__)) + "/DistMult_preTrain/ent_embeddings_n10_d300.npy")).to(device)
